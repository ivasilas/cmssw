...
private:
  ...
  std::vector<int>*   m_tp_nmatch; // number of genuine tracks with 3 or more stubs to which the TP is matched to, hopefully this is always == 0 or 1...
...
void L1TrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( iterL1Track = TTTrackHandle->begin(); iterL1Track != TTTrackHandle->end(); iterL1Track++ ) {
    ...
    float tmp_trk_z0   = iterL1Track->getPOCA().z();
  ...
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
    ...
    if (DebugMode) cout << "Tracking particle, pt: " << tmp_tp_pt << " eta: " << tmp_tp_eta << " phi: " << tmp_tp_phi 
			<< " z0: " << tmp_tp_z0 << " pdgid: " << tmp_tp_pdgid << endl;

    if (MyProcess==13 && abs(tmp_tp_pdgid) != 13) continue;
    if (MyProcess==11 && abs(tmp_tp_pdgid) != 11) continue;
    if (MyProcess==211 && abs(tmp_tp_pdgid) != 211) continue;
    if (MyProcess==6 && abs(tmp_tp_pdgid) != 211) continue;

    if (tmp_tp_pt < 0.2) continue;
    if (fabs(tmp_tp_eta) > 2.5) continue;
    if (fabs(tmp_tp_z0) > 30.0) continue;
    if (MyProcess==6 && tmp_tp_pt < 2.0) continue;
	...
	if (DebugMode) cout << "   ... a matched L1 track, nstubs = " << matchedTracks.at(it)->getStubRefs().size() << endl;
	
	// require at least three stubs for it to be a valid match
	if (matchedTracks.at(it)->getStubRefs().size() < 3) continue;
	nGoodMatch++;
	if (nGoodMatch == 1) i_track = it;
      }
    }
    // ----------------------------------------------------------------------------------------------
    ...
    if (nGoodMatch > 0) {
      ...
      if (DebugMode) cout << "   ... good matched L1 track, pt: " << tmp_matchtrk_pt << " eta: " << tmp_matchtrk_eta << " phi: " << tmp_matchtrk_phi 
			  << " z0: " << tmp_matchtrk_z0 << " chi2: " << tmp_matchtrk_chi2 << " nstub: " << tmp_matchtrk_nstub << endl;
    }
    ...
    m_tp_nmatch->push_back(nGoodMatch);
